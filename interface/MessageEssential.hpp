...
public:
    ...
    void parse(BufferView& cur)
    {
        if (cur.start+sizeof(S) >= cur.limit)
        {
            // return cur.start;
        }

        uint32_t size = *(S*)(cur.start);
...
public:
    ...
    void parse(BufferView& cur)
    {
        uint8_t *nullf= cur.start;
        while (nullf<cur.limit && *(nullf++));
        value = std::string((char*)cur.start, nullf-1-cur.start);
        cur.start = nullf;
    }
...
public:
    ...
    void parse(BufferView& cur)
    {
        uint32_t size = *(uint32_t*)(cur.start);
        if (cur.start+size > cur.limit)
        {
            // return limit;
        }
        value.resize(size);
        std::memcpy(value.data(), cur.start+sizeof(uint32_t), size);
        cur.start += sizeof(uint32_t) + size;
    }
...
public:
    ...
    void parse(BufferView& cur)
    {
        if (cur.start+sizeof(T) > cur.limit)
        {
            // return cur.limit;
        }
        value = *((T*)cur.start);
