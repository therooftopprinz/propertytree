...
class Build:
    ...
    def generate_make(self):
        ...
        if (self.target_type == 0):
            output = output + '\t'+ CXX + ' ' + self.linkflags + ' ' + ' '.join(objects) + ' ' + ' '.join(self.dependencies) +  ' -o ' + self.output_file + '\n'
        else:
...
COMMON_TEST_SOURCES = clean_filenames(r.stdout.readlines())
COMMON_SRC_SOURCES  = clean_filenames(s.stdout.readlines())

print CLIENT_TEST_SOURCES
print CLIENT_SRC_SOURCES
print SERVER_TEST_SOURCES
print SERVER_SRC_SOURCES
print COMMON_TEST_SOURCES
print COMMON_SRC_SOURCES
...
client_test.set_src_dir(TLD+'client/test/')
client_test.add_src_files(CLIENT_TEST_SOURCES)
client_test.add_dependencies(['gtest.a','common_test.a','client.a'])
...
server_test.set_src_dir(TLD+'server/test/')
